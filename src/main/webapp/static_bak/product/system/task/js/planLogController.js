...
app.controller("planLogController", [ "$scope","dialog",'mask','$timeout','http','timeAlert',function(scope,dialog,mask,timeout,http,timeAlert) {
	...
	scope.p={};
	scope.panelClass=[
	                  'panel-danger','panel-success','panel-warning','panel-primary','panel-danger'
	                  ];
	//分页指令的测试数据
	  var vm = scope.vm = {};
	  vm.items = [];
	  ...
	  htt.delAllData={};
	  htt.delAllData.service="taskPlanLogService?delAll";
	  ...
	  scope.delAll=function(){
		  ...
		  mask.showLoading();
		  htt.delAllData.params=[scope.plan];
		  http.callService(htt.delAllData).success(function(data){
			 if(data=='true'){
				 timeAlert.success("已清空");
	  ...
	  scope.queryGridContent = function(){
		  ...
		  htt.planLogData.params.push(scope.plan);
