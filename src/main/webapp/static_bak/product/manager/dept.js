function loadTitle(deptId,flag){
	...
	var data = [ deptId];
	$.callService('deptService','getDepts',data,function(d){
		var html1 = "<div class='title_box'>"+
		" <h2 class='title'>";
		var deptName = "<a href=\"javascript:goMajor("+ d[0].DEPT_ID+");\" style='color:#fff'>"+d[0].DEPT_NAME+"</a>";
		for(var i=0;i<d.length;i++){
			if(i>0 && flag){
				deptName = deptName+" > <a href=\"javascript:goMajor("+d[i].DEPT_ID+");\" style='color:#fff'>"+d[i].DEPT_NAME+"</a>";
			}
		}
		var html2 = "</h2> "+
	...
	var fun = "getDeptTeach";
	if(flag){
		fun = "getMajorTeach";
	}
	$.callService('deptService',fun,data,function(d){
		...
		for(var i=0;i<d.length;i++){
			...
			}else if(d[i].DEPT_LEVEL == 1){
				htmlDept2 = htmlDept2 + "<li><a href=\"javascript:goMajor("+d[i].DEPT_ID+");\">"+
				"<img src ='"+ path +"/static/images/manager/msx.png' /></a>"+
				"<a class='department_name' href='#'>"+ d[i].DEPT_NAME+"<br /> <span class='people_all'> "+d[i].NUMS +" </span> </a></li>";
			} 
		}
		var htmlDept3 = "</ul>"+
