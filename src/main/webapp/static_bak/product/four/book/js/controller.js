	 ...
	 $scope.page = {
	  ...
	  $scope.queryGridContent = function(){
		  ...
		  $.ajax({
				type: "POST",
				async:false,
				url:httpConfig.basePath+"/four/book/detail/"+httpConfig.id,
	  ...
	  $scope.initSource = function(){
		  ...
		  $.ajax({
				type: "POST",
				async:false,
				url:httpConfig.basePath+"/four/book/detail/group/deal/"+httpConfig.id,
				success: function(data){
		  ...
		  $.ajax({
				type: "POST",
				async:false,
				url:httpConfig.basePath+"/four/book/detail/group/time/"+httpConfig.id,
				success: function(data){
	  ...
	  $scope.mutiSource = [{
			queryName : '馆藏类别',// 名称
			queryCode : "store_code",
			items : [ ],// 条件数据
		}, {
			queryName : '借书时间',// 名称
			queryCode : "substr(borrow_time_,0,4)",
			items : [ ],// 条件数据
		} ];
	  $scope.initSource();
